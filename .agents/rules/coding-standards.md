...
## Core Directive
When writing code for Nexora, you must ALWAYS assume the application is operating at an extreme enterprise scale, serving **crores (tens of millions) of requests daily**. Every line of code must be heavily optimized for high concurrency, security, and reusability. 
...
3. **Clean & Reusable**:
   ...
   - Avoid code duplication; extract generic patterns into the `Nexora.Shared` kernel.
4. **Resiliency**:
