...
3. Once synced, the `Shipwise.Modules.Identity` maps the Keycloak `sub` to a local `UserId` and `TenantId`.
4. The `TenantId` is stored in a scoped `ICurrentUserContext` (in `Shipwise.Shared`).
